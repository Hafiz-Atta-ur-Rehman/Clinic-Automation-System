    ...
    <div class="col-lg-10 col-md-10 col-xs-10 col-xs-offset-1 col-md-offset-1 col-lg-offset-1">
        <form class="form-horizontal" enctype="multipart/form-data" method="post" action="<?= site_url('index.php/doctor/manage_reports'); ?>">
            <div class="form-group" id="data_1">
                ...
                <div class="col-lg-10">
                    <div class="input-group date">
                        <span class="input-group-addon"><i class="fa fa-calendar"></i></span><input type="text" autocomplete="off" name="date" required class="form-control" value="<?= isset($_POST['date']) ? $_POST['date'] : $record->date; ?>">
                    </div>
                </div>
            </div>
            <div class="form-group"><label class="col-lg-2 control-label">Type</label>
                <div class="col-lg-10">
                    <select required data-placeholder="Choose a Country..." class="select chosen-select" name="report_id" tabindex="2">
                        <option value="">Select</option>
                        <option value="operation_report" <?= $record->report_id == 'operation_report' ? 'selected' : ''; ?>>Operation</option>
                        <option value="birth_report" <?= $record->report_id == 'birth_report' ? 'selected' : ''; ?>>Birth</option>
                        <option value="death_report" <?= $record->report_id == 'death_report' ? 'selected' : ''; ?>>Death</option>
                    </select>
                </div>
            </div>
            <div class="form-group"><label class="col-lg-2 control-label">Select Patient</label>
                <div class="col-lg-10">
                    <select required data-placeholder="Choose a Country..." class="select chosen-select" name="patient_id" tabindex="2">
                        <option value="">Select</option>
                        <?php foreach ($record->patients as $patient) : ?>
                            <option value="<?= $patient->id; ?>" <?php echo $patient->id == $record->patient_id ? 'selected' : ''; ?>><?= $patient->name; ?></option>
                        <?php endforeach; ?>
                    </select>
                </div>
            </div>
            <div class="form-group"><label class="col-lg-2 control-label">Description</label>
                <div class="col-lg-10">
                    <textarea required name="description" class="form-control" rows="5"><?= isset($_POST['description']) ? $_POST['description'] : $record->description; ?></textarea>
                </div>
            </div>
